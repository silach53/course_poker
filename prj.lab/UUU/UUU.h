...
void ask_for_move(std::pair<const int,player>){
...
public:
	...
	void game(int number_of_players = 6) {
		...
		while(one_left()){
			...
			while(turn_is_active){
				for(auto p: players){
					...
					//Тут надо запросить у другого кода, при фиксированно состоянии игры сделать ход
					ask_for_move(p);
